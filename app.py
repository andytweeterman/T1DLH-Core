...
try:
    ...
    st.error("⚠️ API Critical Failure. Please try again later.")
    st.stop()
...
try:
    ...
    st.error("⚠️ Data loading failed. Please check your connection and try again.")
    ...
    if audio_hash != st.session_state.get("last_audio_hash"):
        ...
        with st.spinner("Processing Voice Dump and Correlating Telemetry..."):
            try:
                ...
                st.error("⚠️ Voice Analysis failed. Please try again.")
...
if st.session_state.active_view == "Daily Briefing":
    ...
    with st.spinner("Compiling Executive Briefing..."):
        try:
            ...
            st.error("⚠️ Failed to generate briefing. Please check API connection.")
        ...
        if st.form_submit_button("Analyze Correlation", type="primary") and text_input:
            with st.spinner("Correlating subjective report with objective telemetry..."):
                try:
                    ...
                    st.error("⚠️ Correlation Analysis failed. Please try again.")
