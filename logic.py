...
def get_whoop_risk_modifier(whoop_metrics):
    ...
    # BUGFIX: Robustly parse nested Whoop cycle schema to grab true Recovery
    if 'recovery' in whoop_metrics or 'sleep' in whoop_metrics:
        recovery = whoop_metrics.get('recovery', {}).get('score', {}).get('recovery_score', 100)
        strain = whoop_metrics.get('score', {}).get('strain', 0.0)
        sleep = whoop_metrics.get('sleep', {}).get('score', {}).get('sleep_performance_percentage', 100)
    else:
        # Fallback if dictionary is already flattened
