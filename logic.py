...
def fetch_health_data():
    """Generates 24 hours of Dexcom/Whoop data."""
    now = datetime.now()
...
def fetch_environmental_load(lat=38.9847, lon=-77.0947, api_key=""):
    ...
    (Default coordinates set to Bethesda, MD)
    """
    if not api_key:
    ...
    try:
        ...
        # Severe Weather conditions (e.g., Thunderstorms: IDs 200-299)
...
# -----------------------------------------------------------------------------
def calc_glycemic_risk(df, context, whoop_data=None, meeting_count=0, speaker_mode=False, owm_api_key=""):
    ...
    # Context Appending for Claude
    final_reason = f"{whoop_status} | {sched_status} | {env_status}"
    
    # Hypoglycemia Preemption (Heat)
    ...
    # Compound Load Escalation (Cold/Storms + Whoop)
    # We calculate the combined excess strain above the 1.0 baselines
